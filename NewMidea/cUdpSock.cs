...
namespace System
{
        ...
        private void udpListen()//
        {
            try
            {
                ...
                while (!isOutClass)
                {
                    ...
                    {
                        mDataReciveByte(null, new RecieveByteArgs(ie.Address.ToString(), b));
                    }
                    string s = Encoding.GetEncoding("gb2312").GetString(b, 0, b.Length);
                    if (mDataReciveString != null)
                    {
                        mDataReciveString(null, new RecieveStringArgs(ie.Address.ToString(), s));
                    }
                }
            }
            catch
    ...
    public class RecieveStringArgs : EventArgs
    {
        ...
        public RecieveStringArgs(string mRemostHost, string mReadStr)
        {
    ...
    public class RecieveByteArgs : EventArgs
    {
        ...
        public RecieveByteArgs(string mRemostHost, byte[] mReadByte)
        {
            RemostHost = mRemostHost;
            ReadByte = mReadByte;
        }
