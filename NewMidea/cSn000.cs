    ...
    class cSn000
    {
        ...
        public void Send(string send)
        {
            if (comPort != null)
            {
                ...
                {
                    send = send.Trim();
                    byte[] bb = new byte[(int)Math.Ceiling(send.Length / 2.000)];
                    for (int i = 0; i < send.Length; i = i + 2)
                    {
                        bb[i / 2] = Convert.ToByte(send.Substring(i, 2), 16);
                    }
                    sendDataCount = bb.Length;
                    comPort.DiscardInBuffer();
                    comPort.Write(bb, 0, bb.Length);
                }
            }
        }
        /// <summary>
